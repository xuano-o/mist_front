...
<template>
  <div class="toolbar-container flex items-center justify-between">
    ...
    <div v-if="['side', 'single', 'only-side', 'side-panel'].includes(settingsStore.settings.menu.mode) || settingsStore.mode === 'mobile'" class="h-full flex flex-shrink-0 items-center justify-end px-2">
      <RightSide />
    </div>
  </div>
</template>
